...
var ImageBuffer = function (wide, tall) {
...
ImageBuffer.prototype.PATTERN_L_SHAPE = 0;
ImageBuffer.prototype.PATTERN_LIGHT_ORANGE = 1;

//==============================================================================
// Replace current 8-bit RGB contents of 'imgageBuffer' with a colorful pattern
...
ImageBuffer.prototype.setTestPattern = function(pattern) {
  ...
  for(var j=0; j< this.ySiz; j++) {                     // for the j-th row of pixels
  	for(var i=0; i< this.xSiz; i++) {                   // and the i-th pixel on that row,
  		...
  		switch(pattern) {
  			case this.PATTERN_L_SHAPE: //================(Colorful L-shape)============================
  				...
  				break;
  			  case this.PATTERN_LIGHT_ORANGE: //================(bright orange)===============================
...
ImageBuffer.prototype.int2float = function() {
...
ImageBuffer.prototype.float2int = function() {
	for(var j=0; j< this.ySiz; j++) {       // for each scanline
		for(var i=0; i< this.xSiz; i++) {   // for each pixel on that scanline
			var idx = (j*this.xSiz + i)*this.pixSiz;// Find array index at pixel (i,j)
				// find 'clamped' color values that stay >=0.0 and <=1.0:
			var red = Math.min(1.0, Math.max(0.0, this.fBuf[idx]));
			var green = Math.min(1.0, Math.max(0.0, this.fBuf[idx+1]));
			var blue = Math.min(1.0, Math.max(0.0, this.fBuf[idx+2]));
			...
			this.iBuf[idx   ] = Math.min(255,Math.floor(red*256.0));
			this.iBuf[idx +1] = Math.min(255,Math.floor(green*256.0));
			this.iBuf[idx +2] = Math.min(255,Math.floor(blue*256.0));
		}
  }
}
